    ...
    <div class="py-6">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form method="POST" action="{{ route('signatures.store') }}" class="space-y-6">
                        @csrf
                                ...
                                <select id="account_id" 
                                    ...
                                    @foreach($accounts as $account)
                                        <option value="{{ $account->id }}" 
                                                {{ old('account_id', $accountId) == $account->id ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <select id="template_type" 
                                ...
                                <option value="{{ $key }}" {{ old('template_type', $templateType) == $key ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <div class="border border-gray-200 rounded-md p-4 bg-gray-50">
                                <div id="template-preview" class="signature-preview">
                                    {!! $selectedTemplate['html'] !!}
                                </div>
                            </div>
                            <p class="mt-1 text-sm text-gray-500">
                                This is a preview of the selected template. You can customize it in the HTML editor below.
                            </p>
                        </div>
                        @endif
                        ...
                        <div>
                            ...
                            @enderror
                            <p class="mt-1 text-sm text-gray-500">
                                Upload images for your signature (max 2MB each). Supported formats: JPG, PNG, GIF, WebP.
                            </p>
                        </div>
                            ...
                            <div class="mt-1">
                                <div id="html-editor" class="border border-gray-300 rounded-lg min-h-[400px] focus-within:border-blue-500 focus-within:ring-1 focus-within:ring-blue-500 shadow-sm">
                                    ...
                                    <div id="html-editor-content" 
                                         ...
                                         style="font-family: 'Aptos Display', Arial, sans-serif; font-size: 15px; line-height: 1.4;">{{ old('html_content') }}</div>
                                </div>
                                <textarea id="html_content" 
                                          name="html_content" 
                                          class="hidden">{{ old('html_content') }}</textarea>
                            </div>
                            @error('html_content')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
    ...
    <script>
        ...
        function loadTemplate(templateType) {
            if (templateType === 'custom') {
                document.getElementById('template-preview').innerHTML = '<p class="text-gray-500 text-center">No template selected</p>';
                return;
            }

            fetch(`{{ route('signatures.template-preview') }}?template=${templateType}`)
                .then(response => response.json())
                .then(data => {
                    if (data.success) {
                        document.getElementById('template-preview').innerHTML = data.template.html;
                        
                        // Add CSS styles
                        const style = document.createElement('style');
                        style.textContent = data.template.css;
                        document.head.appendChild(style);
                        
                        // Load template into editor if it's not custom
                        if (templateType !== 'custom' && data.template.html) {
                            document.getElementById('html-editor-content').innerHTML = data.template.html;
                            updateHtmlContent();
                        }
                    }
                })
                .catch(error => {
                    ...
                    document.getElementById('template-preview').innerHTML = '<p class="text-red-500 text-center">Error loading template</p>';
                });
        }
